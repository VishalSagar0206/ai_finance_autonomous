...
with st.sidebar:
    ...
    tickers_input = st.text_input(
        "Tickers (comma separated)", value="RELIANCE, AAPL, GOOG"
    )
    time_horizon = st.selectbox(
        "Time Horizon", ["1 Month", "6 Months", "1 Year", "2 Years"]
    )
...
if "hitl_ready" in st.session_state and st.session_state.hitl_ready:
    ...
    with col_st:
        ...
        if stress:
            ...
            st.write(
                f"Resilient: {'✅ Yes' if stress.get('is_resilient') else '❌ No'}"
            )
    ...
    if st.button(
        "🚨 CONFIRM & EXECUTE ORDERS", type="primary", use_container_width=True
    ):
