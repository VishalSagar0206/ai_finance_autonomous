...
with st.sidebar:
    ...
    st.markdown("---")
    if st.button(
        "🚀 INITIATE NEURAL ORCHESTRATION", type="primary", width='stretch'
    ):
    ...
    if config.OFFLINE_MODE:
        st.sidebar.warning("🟡 Offline / Mock Mode")
        st.sidebar.caption("Agents are using mock fallbacks. No live APIs are active.")
    else:
        st.sidebar.success("🟢 Live Mode")
        st.sidebar.caption("Agents may use live APIs if valid keys are configured.")

    st.sidebar.markdown("#### API Status")

    llm_live = config.has_live_llm()
    market_live = config.has_live_market_data()
    broker_live = config.has_live_broker()

    st.sidebar.write(f"LLM / Gemini: {'🟢 Live' if llm_live else '⚪ Mock'}")
    st.sidebar.write(f"Market Data: {'🟢 Live' if market_live else '⚪ Mock'}")
    st.sidebar.write(f"Broker / Alpaca: {'🟢 Live' if broker_live else '⚪ Mock'}")
...
if config.OFFLINE_MODE:
    st.warning("🟡 Running in Offline Mode — no live API execution.")
else:
    st.success("🟢 Running in Live Mode.")
...
if st.session_state.get("hitl_ready") or st.session_state.get("finished"):
    ...
    if final_report:
        st.success(
            "✅ **Full Workflow Complete.** SOR Execution and Final Report finished successfully."
        )
        ...
        last_feedback = (
            feedback[-1] if feedback else "No detailed rejection reason recorded."
        )
        st.error(
