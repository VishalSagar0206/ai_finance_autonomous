...
def _message_from_results(results: Dict[str, Any]) -> str:
    if results.get("status") == "error":
        return results.get("error") or results.get("stderr") or "Backtest execution error"
    return "SUCCESS"
...
def coder_agent(state: ADKState) -> Dict[str, Any]:
    ...
    if not state.draft_strategy:
        return {
            ...
            "execution_logs": list(state.execution_logs or []) + ["No strategy available."],
        }

    if not config.has_live_llm():
        logger.warning("Coder: live LLM disabled. Using deterministic backtest template.")
        code = QuantCoder.generate_backtest_code(state)
        results = code_executor.execute_python_code(code)
        return {
            "backtest_results": results,
            "backtest_attempts": state.backtest_attempts + 1,
            "execution_logs": list(state.execution_logs or []) + [_message_from_results(results)],
        }
