...
def strategy_generator_agent(state: ADKState) -> Dict[str, Any]:
    ...
    if not os.environ.get("GOOGLE_API_KEY"):
        ...
        successful_tickers = [t for t in tickers if get_status(fund_results.get(t)) == "completed"]
        
        if not successful_tickers:
            return {"draft_strategy": DraftStrategy(
        ...
        for t in successful_tickers:
            ...
            score = 2
            if "negative" in q_insight: score -= 2
            if score <= 1.1: ticker_scores[t] = 0
            else:
