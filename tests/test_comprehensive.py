...
def test_scenario_4_defensive_fallback_on_total_data_failure(config):
    ...
    assert final_state["approval_status"] == ApprovalStatus.APPROVED # Defensive strategies are approved by Critic
...
def test_scenario_7_conflicting_signals_resolution(config):
    ...
    initial_state.observations.quantitative = {
        "results": {
            ...
            "TSLA": {"insight": "Significant negative momentum.", "status": "completed"} # Bearish signal
        }
    }
